...
export const createStockRecord = async (req, res) => {
  try {
    ...
    const stockRecord = new StockRecord({
      ...
      totalValue: req.body.stockQuantity * req.body.unitPrice,
...
export const getAllStockRecords = async (req, res) => {
  try {
    ...
    const filter = {};
    if (req.query.stateLocation) filter.stateLocation = req.query.stateLocation;
...
export const updateStockRecord = async (req, res) => {
  try {
    const updatedRecord = await StockRecord.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
...
export const deleteStockRecord = async (req, res) => {
  try {
    const deletedRecord = await StockRecord.findByIdAndDelete(req.params.id);
    if (!deletedRecord)
      return res.status(404).json({
        success: false,
        message: "Stock record not found",
...
export const getStockSummaryByState = async (req, res) => {
  try {
